-- Migration 202609060001: drop the two personal-name entries from the
-- company blacklist in finalize_email_classification.
--
-- 202609040003 hardcoded the app's original single owner ('andrei gheorghe',
-- 'gheorgheandrei13') into the "company extracted is actually a misparse of
-- the applicant's own name" guard. That doesn't generalize to a public repo,
-- and there's no per-owner config this RPC can read at low cost — so the
-- entries are just dropped. A fork that hits the same misparse can add their
-- own name back here the same way.
--
-- create or replace: the complete function, copy-pasted from 202609040003
-- with only that one line changed (see CLAUDE.md's note on this function).
...
begin
  ...
  if resolved_classification in ('unrelated', 'conference') then
    ...
    'smartrecruiters', 'greenhouse', 'workable', 'ashby', 'lever',
