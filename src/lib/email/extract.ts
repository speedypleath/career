...
export function extractCompanyName(subject: string, sender: string, body: string): string {
  ...
  m = subject.match(/(?:applying to join|applying to|application to|interest in joining|interest in|application with)\s+([A-Za-z0-9\s._&-]+?)(?:!|\.|\(|$|\s+team|\s+hiring|\s+andrei)/i)
  if (m && m[1].trim().length > 1) {
    const s = sanitizeCompany(m[1])
    if (s !== "Unknown Company") return s
  }
